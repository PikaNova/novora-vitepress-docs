...
本章优先使用官方仓库的一键部署按钮。开始前应已完成 Neon 项目创建，并准备好 Pooled connection string、管理员初始密码和应急恢复密钥。
...
8. 在 `ADMIN_RECOVERY_KEY` 中填写至少 16 位、且与登录密码不同的应急恢复密钥。
9. 点击 **Deploy**，等待构建完成。

三个变量会在一键部署表单中一次填写，无需为恢复密钥额外重新部署。完整变量说明见下一章。
